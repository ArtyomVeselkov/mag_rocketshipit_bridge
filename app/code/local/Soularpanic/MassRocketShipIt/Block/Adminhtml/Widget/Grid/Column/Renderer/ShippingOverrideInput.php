...
class Soularpanic_MassRocketShipIt_Block_Adminhtml_Widget_Grid_Column_Renderer_ShippingOverrideInput
    ...
    function _paymentChecksOut($order) {
        ...
        $avs = false;
        $cvv2 = false;
        if (array_key_exists('paypal_avs_code', $payment_info)) {
            $avs = $payment_info['paypal_avs_code'];
            $cvv2 = $payment_info['paypal_cvv2_match'];
        }
        if (array_key_exists('avsPostalCodeResponseCode', $payment_info)) {
            $avs = $payment_info['avsPostalCodeResponseCode'];
            $cvv2 = $payment_info['cvvResponseCode'];
        }
